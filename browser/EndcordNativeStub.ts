...
// Only import pure modules that don't import other parts of Endcord.
...
const setCssDebounced = debounce((css: string) => EndcordNative.quickCss.set(css));

const themeStore = DataStore.createStore("EndcordThemes", "EndcordThemeData");

// probably should make this less cursed at some point
window.EndcordNative = {
    themes: {
    ...
    updater: {
        getRepo: async () => ({ ok: true, value: "https://github.com/rootpoii/endcord" }),
    ...
    quickCss: {
        get: () => DataStore.get("EndcordQuickCss").then(s => s ?? ""),
        set: async (css: string) => {
            await DataStore.set("EndcordQuickCss", css);
        ...
        async openEditor() {
            ...
            const win = open("about:blank", "EndcordQuickCss", features);
            if (!win) {
            ...
            win.getCurrentCss = () => EndcordNative.quickCss.get();
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem("EndcordSettings") || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem("EndcordSettings", JSON.stringify(s)),
